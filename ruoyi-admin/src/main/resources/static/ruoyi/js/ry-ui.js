...
(function ($) {
    $.extend({
        ...
        operate: {
            ...
            createMission: function (id)
            {
                table.set();
                $.modal.open("设置定时任务",$.operate.createMissionUrl(id));
            },
            // 添加信息，以tab页展现
            addTab: function (id) {
                table.set();
            ...
            createMissionUrl:function (id){
                var url = $.common.isEmpty(id) ? table.options.missionUrl.replace("{id}", "") : table.options.missionUrl.replace("{id}", id);
                return url;
            },
            // 修改信息
            edit: function(id) {
